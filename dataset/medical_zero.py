...
class MedTrainDataset(Dataset):
    def __init__(
        self, dataset_path="/data/", class_name="Brain", resize=240, batch_size=1
    ):
        ...
        for class_name_one in CLASS_NAMES:
            if class_name_one != self.class_name:
                ...
                img_dir = os.path.join(
                    self.dataset_path, f"{class_name_one}_AD/test/good/img"
                )
                for f in os.listdir(img_dir):
                    data_img[class_name_one].append((os.path.join(img_dir, f), 0))

                img_dir = os.path.join(
                    self.dataset_path, f"{class_name_one}_AD/test/Ungood/img"
                )
                for f in os.listdir(img_dir):
        ...
        for class_name_one in data_img.keys():
            if CLASS_INDEX[class_name_one] < 0:
                for image_index in range(
                    0, len(data_img[class_name_one]), self.batch_size
                ):
                    file_path = []
                    img_label = []
                    for batch_count in range(0, self.batch_size):
                        if image_index + batch_count >= len(data_img[class_name_one]):
                            break
                        file_path.append(
                            data_img[class_name_one][image_index + batch_count][0]
                        )
                        img_label.append(
                            data_img[class_name_one][image_index + batch_count][1]
                        )
                    data.append(
                        [file_path, img_label, None, CLASS_INDEX[class_name_one]]
                    )
            else:
                for image_index in range(
                    0, len(data_img[class_name_one]), self.batch_size
                    ...
                    for batch_count in range(0, self.batch_size):
                        if image_index + batch_count >= len(data_img[class_name_one]):
                            break
                        single_file_path = data_img[class_name_one][
                            image_index + batch_count
                        ][0]
                        single_label = data_img[class_name_one][
                            image_index + batch_count
                        ][1]
                    ...
                    data.append(
                        [file_path, img_label, gt_path, CLASS_INDEX[class_name_one]]
                    )
        random.shuffle(data)
        return data
...
class MedTestDataset(Dataset):
    def __init__(self, dataset_path="/data/", class_name="Brain", resize=240):
        assert class_name in CLASS_NAMES, "class_name: {}, should be in {}".format(
            class_name, CLASS_NAMES
        )
    ...
    def load_dataset_folder(self, seg_flag):
        ...
        img_fpath_list = sorted(
            [os.path.join(abnorm_img_dir, f) for f in os.listdir(abnorm_img_dir)]
        )
        x.extend(img_fpath_list)
        ...
        if seg_flag > 0:
            ...
            gt_fpath_list = sorted(
                [os.path.join(gt_type_dir, f) for f in os.listdir(gt_type_dir)]
            )
