...
from open_clip_custom.tokenizer import SimpleTokenizer
from open_clip import get_tokenizer
from .config import biomedclip_hf_api
...
tokenizer = get_tokenizer(biomedclip_hf_api)


class TextEncoder(nn.Module):
    def __init__(self, biomedclip):
...
class PromptLearner(nn.Module):
    def __init__(
        ...
        for cls in prompts:
            for position in class_token_position:
                if CSC:
                    ...
                    ctx_vectors = torch.empty(n_ctx, ctx_dim).to(biomedclip.device)
        ...
        tokenized_prompts = {
            cls: torch.cat(
                [tokenizer(prompt) for prompt in prompts_learnable_tokens[cls]]
            ).to(biomedclip.device)
            for cls in prompts_learnable_tokens
        }

        with torch.no_grad():
            embeddings = {
                cls: biomedclip.text.transformer.embeddings.word_embeddings(tokenized_prompts[cls])
...
class PromptMaker(nn.Module):
    ...
    def __init__(
        ...
        self.prompt_learner = PromptLearner(
            prompts, n_ctx, CSC, class_token_position, biomedclip
        )
