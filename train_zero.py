...
from CLIP.TEncoderCLIP import TEncoderCLIP
from CLIP.VEncoderCLIP import VEncoderCLIP
...
CLASS_INDEX_INV = {3: 'Brain', 2: 'Liver', 1: 'Retina_RESC', -1: 'Retina_OCT2017', -2: 'Chest',
                   -3: 'Histopathology'}  # ,
...
def main():
    ...
    parser.add_argument("--text_learning_rate", type=float, default=0.001, help="learning rate")
    parser.add_argument("--vision_learning_rate", type=float, default=0.0001, help="learning rate")
    ...
    # custom TEncoderCLIP
    t_encoder_clip = TEncoderCLIP(clip_model=clip_model, prompt_parameters=prompt_parameters)
    t_encoder_clip.to(device)
    t_encoder_clip.eval()

    # custom VEncoderCLIP
    v_encoder_clip = VEncoderCLIP(clip_model=clip_model, features=args.features_list)
    v_encoder_clip.to(device)
    v_encoder_clip.eval()
    for name, param in t_encoder_clip.named_parameters():
        param.requires_grad = True

    # prompt learner
    prompt_learner = AnomalyCLIP_PromptLearner(device=device, model=t_encoder_clip, prompt_parameters=prompt_parameters)
    prompt_learner.to(device)

    # optimizer
    text_optimizer = torch.optim.Adam(list(prompt_learner.parameters()), lr=args.text_learning_rate, betas=(0.5, 0.999))
    seg_optimizer = torch.optim.Adam(list(v_encoder_clip.seg_adapters.parameters()), lr=args.vision_learning_rate,
                                     betas=(0.5, 0.999))
    det_optimizer = torch.optim.Adam(list(v_encoder_clip.det_adapters.parameters()), lr=args.vision_learning_rate,
                                     betas=(0.5, 0.999))
    ...
    for epoch in range(args.epoch):
        ...
        if epoch != 0:
            print("\ttest:")
            score = test(args, v_encoder_clip, t_encoder_clip, prompt_learner, test_loader)
            if score >= save_score:
                save_score = score
                ckp_path = f'{args.ckpt_path}/zero-shot/{args.obj}_epoch_{epoch}.pth'
                torch.save({'seg_adapters': v_encoder_clip.seg_adapters.state_dict(),
                            'det_adapters': v_encoder_clip.det_adapters.state_dict(),
        ...
        for (image, image_label, mask, seg_idx) in tqdm(train_loader):
            ...
            with torch.cuda.amp.autocast():
                image_features, seg_patch_tokens, det_patch_tokens = v_encoder_clip.encode_image_learn(image)
                image_features = image_features / image_features.norm(dim=-1, keepdim=True)
                seg_patch_tokens = [p[:, 1:, :] for p in seg_patch_tokens]
                det_patch_tokens = [p[:, 1:, :] for p in det_patch_tokens]

                prompts, tokenized_prompts, compound_prompts_text = prompt_learner(cls_id=None)
                text_features = t_encoder_clip.encode_text_learn(prompts, tokenized_prompts, compound_prompts_text).float()
                ...
                if seg_idx > 0:
                    ...
                    loss = text_probs_loss + det_loss + seg_loss
...
def test(args, v_encoder_clip, t_encoder_clip, prompt_learner, test_loader):
    gt_list = []
    gt_mask_list = []
    image_scores = []
    segment_scores = []
    ...
    for (image, y, mask) in tqdm(test_loader):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            image_features, ori_seg_patch_tokens, ori_det_patch_tokens = v_encoder_clip.encode_image_learn(image)
