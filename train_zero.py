...
def main():
    ...
    prompt_optimizer = torch.optim.Adam(list(prompt_maker.prompt_learner.parameters()), lr=args.learning_rate,
                                        betas=(0.5, 0.999))
    ...
    checkpoint = torch.load(os.path.join(f'/content/drive/MyDrive/Thesis/bubuchacha_2/zero-shot/Liver_7.pth'))
    ...
    for epoch in range(args.epoch):
        ...
        for (image, image_label, mask, seg_idx) in tqdm(train_loader):
            ...
            with torch.cuda.amp.autocast():
                _, [seg_patch_tokens, det_patch_tokens], [seg_patch_outs, det_patch_outs] = model(image)
...
def test(args, seg_model, test_loader, prompt_maker):
    ...
    for (image, y, mask) in tqdm(test_loader):
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            _, [seg_patch_tokens, det_patch_tokens], [seg_patch_outs, det_patch_outs] = seg_model(image)
